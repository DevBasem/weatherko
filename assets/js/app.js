 ...
 * @param {NodeList} elements => Elements Node List
 * @param {string} eventType => Event Type e.g.: "click", "mouseover"
 * @param {function} callback => Execution on action function
 */
      ...
      fetchData(url.geo(searchField.value), function (locations) {
        ...
        for (const { name, lat, lon, country, state } of locations) {
          ...
          searchItem.innerHTML = `
            ...
            <a class="item-link has-state" href="#/weather?lat=${lat}&lon=${lon}" aria-label="${name} weather" data-search-toggler></a>
            `;
...
/**
 * Render Updated Weather Data In HTML
    ...
    fetchData(url.forecast(lat, lon), function(forecast){
        ...
        for (let i = 7, len = forecastList.length; i < len; i+=8)
        {
            ...
            <div class="icon-wrapper">
                ...
                <span class="span">
                  <p class="tile-2">${parseInt(temp_max)}&deg;</p>
                </span>
            </div>
